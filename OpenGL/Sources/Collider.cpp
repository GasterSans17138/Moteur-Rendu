...
void SphereCollider::Render() const
{
	...
	glUniform1i(glGetUniformLocation(Renderer::shaderProgram, "material.useTexture"), false);
	glUniform3f(glGetUniformLocation(Renderer::shaderProgram, "material.albedoColor"), 0, 1, 0);

	glBindVertexArray(CollisionMesh::Sphere.VAO);
	glUniformMatrix4fv(glGetUniformLocation(Renderer::shaderProgram, "mvp"), 1, GL_TRUE, &mvp.value[0][0]);
	glUniformMatrix4fv(glGetUniformLocation(Renderer::shaderProgram, "model"), 1, GL_TRUE, &model.value[0][0]);
